BEGIN TRANSACTION
/************************** DELETE OLD JOB *******************************************/
IF EXISTS (select * FROM msdb.dbo.sysjobs_view where name =N'LoadDataWarehouse_AssetType')
BEGIN
...
SET @ReturnCode = 0
--SET @v_PackageFolderPath='\\hou150w8ahs012\GTR4\DM\FullData\'

--SET @v_ConfigFilePath='\\hou150w8ahs012\GTR4\DM\FullData\Config\'
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'LoadDataWarehouse_AssetType', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'The job loads the AssetType ExcelSheet Data to Stage and DW databases.', 
