...
Go
/****** Object:  StoredProcedure [stg].[spValidatePortfolioMaster]    Script Date: 04/04/2016 10:58:13 p. m. ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRY
	...
	+ CASE WHEN ([ACCOUNT_Name] IS NULL) OR RTRIM(LTRIM([ACCOUNT_Name])) = SPACE(0) THEN 'The Account Name value ''' + ISNULL([ACCOUNT_ID], '') + ''' is invalid.' else '' end
	...
	+ CASE WHEN TRY_CONVERT(int, [Hurdle_Rate]) IS NULL THEN 'The Hurdle Rate value ''' + ISNULL([Hurdle_Rate], '') + ''' is invalid.' else '' end
	+ CASE WHEN TRY_CONVERT(int, [Pooled]) IS NULL THEN 'The Pooled value ''' + ISNULL([Pooled], '') + ''' is invalid.' else '' end
	+ CASE WHEN TRY_CONVERT(datetime, [Acct_Inception_Date]) IS NULL THEN 'The Account Inception date value ''' + ISNULL([Acct_Inception_Date], '') + ''' is invalid.' else '' end
	+ CASE WHEN TRY_CONVERT(datetime, [Strat_Inception_Date]) IS NULL THEN 'The strategy Inception date value ''' + ISNULL([Strat_Inception_Date], '') + ''' is invalid.' else '' end
	+ CASE WHEN LEN(RTRIM(LTRIM([Entity_Type]))) > 50 then ' The Entity Type value ''' + ISNULL([Entity_Type], '') + ''' is invalid.' else '' end
	...
	SELECT * FROM @PortfolioMaster WHERE Error = '' ORDER BY CreatedDate ASC
